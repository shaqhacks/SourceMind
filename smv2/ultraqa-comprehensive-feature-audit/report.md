...
- Final QA date: 2026-08-09.
- Scope: Tasks 1-6 historical release gate plus Task 7 unified flashcard review E2E coverage.
- Status: HISTORICAL TASK 6 PASS. CURRENT TASK 7 UNIFIED FLASHCARD REVIEW STATUS: E2E PASS, backend focused slice PASS x3, frontend static/unit PASS, and full release gate PASS. npm audits remain UNAVAILABLE in this addendum because Task 7 did not rerun audit commands.
...
| UX-10 | Student | Flashcard browse, edit/delete, generate, review, grade, queue | Task 7 Playwright desktop/mobile flashcard-review-scopes matrix; frontend unit suite | Course all, chapter all, inline grading, completed replay, and grade failure recovery work with keyboard and strict guards | `flashcard-review-scopes.spec.ts` passed `10/10`; full Vitest passed `102 files / 839 tests` after updating the reader test API mock for `getReviewQueue` | PASS | `rtk npm --prefix frontend run test:e2e -- flashcard-review-scopes.spec.ts`; `rtk npm --prefix frontend test -- --run` | generated `frontend/test-results` removed before commit |
...
| REL-04 | Maintainer | Full suite, repeated sensitive tests, hidden skips, misleading success | Build/lint/repeats/e2e/runtime enforcement | Deterministic green for Task 7 browser, backend, frontend, and build gates | Current Task 7 verification from HEAD `b9c2868`: Playwright `10 passed`; backend review suite `44 passed` on three consecutive runs; frontend typecheck and lint passed; full Vitest `102 files / 839 tests`; `rtk ./build.sh` passed with backend `884 passed, 31 warnings`, generated artifacts drift check, frontend typecheck, Vitest `839 passed`, Next production build, final `BUILD OK`. npm audits were not rerun for this addendum. | PASS for current Task 7 release gate; npm audit UNAVAILABLE | see Commands Run and Task 7 unified flashcard review addendum | none |
...
- `[0] Task 7 Back-to-review/missed replay scenario`: `rtk npm --prefix frontend run test:e2e -- flashcard-review-scopes.spec.ts --project=chromium -g "completed review returns to chooser and replays exact missed cards"` -> `1 passed`.
- `[0] Task 7 inline-reader axe scenario`: `rtk npm --prefix frontend run test:e2e -- flashcard-review-scopes.spec.ts --project=chromium -g "student grades a revealed card inside the chapter reader"` -> `1 passed`.
- `[0] Task 7 full browser matrix`: `rtk npm --prefix frontend run test:e2e -- flashcard-review-scopes.spec.ts` -> `10 passed` across Chromium desktop and mobile.
- `[0] Task 7 backend review suite run 1`: `rtk uv run pytest -q tests/test_review.py tests/test_learner_scoping.py tests/test_review_availability_service.py` from `backend/` -> `44 passed, 8 warnings`.
- `[0] Task 7 backend review suite run 2`: same command -> `44 passed, 8 warnings`.
- `[0] Task 7 backend review suite run 3`: same command -> `44 passed, 8 warnings`.
- `[0] Task 7 frontend typecheck`: `rtk npm --prefix frontend run typecheck` -> `tsc --noEmit`, exit 0.
- `[0] Task 7 frontend lint`: `rtk npm --prefix frontend run lint` -> `eslint`, exit 0.
- `[1 then 0] Task 7 full Vitest`: first `rtk npm --prefix frontend test -- --run` had no failed assertions but exited nonzero from unhandled `course-reader.test.tsx` mock errors (`getReviewQueue` missing from the API mock). Minimal test-only fix added the mock, default empty queue result, and a regression assertion for `getReviewQueue("course-1", { scope: "all", chapterLabel: "Chapter 1", limit: 200 })`. Rerun -> `102 files / 839 tests passed`.
- `[0] Task 7 release gate`: after stopping the repo-local Next dev server on `:3000`, `rtk ./build.sh` -> backend `884 passed, 31 warnings`; OpenAPI export and generated artifact drift check passed; frontend typecheck passed; frontend Vitest `102 files / 839 tests passed`; Next production build passed; final `BUILD OK`.
...
## Task 7 Unified Flashcard Review Addendum

- Date: 2026-08-09.
- Scope: deterministic E2E coverage for unified flashcard review course scope, chapter scope, inline reader grading, completed-session return/replay, and grade failure recovery.
- Brief source: `task-7-brief.md` was not present in this checkout; the available Task 7 requirements were read from `docs/superpowers/plans/2026-08-07-unified-flashcard-review.md` plus controller handoff instructions.
- Browser result: `flashcard-review-scopes.spec.ts` passed in Chromium desktop and mobile: `10 passed (9.3s)`.
- Course-wide coverage: review starts from `/review?course=course-flashcard-review&scope=all`, shows `2 due · 1 new`, grades due/new/future cards by keyboard, and asserts the not-due future card content appears and is graded.
- Chapter coverage: `/flashcards` browse flow verifies the exact encoded chapter review link, starts `scope=all&chapter=Chapter%202%3A%20Scope%20%26%20Replay`, and asserts only the two chapter cards are requested.
- Inline reader coverage: source reader reveals a card with keyboard Enter, passes the critical axe gate after the reviewed Sidebar ID fix, grades the revealed future card as Hard, and confirms the chapter-scoped review metadata request uses `scope=all`, `chapter_label`, and `limit=200`.
- Completed replay coverage: a completed-session snapshot with exact `againCardIds` hydrates the completion view, `Back to review` returns to the chooser without a remount dependency, and `Review missed (2)` requests the exact card ID order before replaying those cards.
- Failure recovery coverage: a deliberately mocked first grade failure keeps the same revealed card visible, shows `Could not save this grade. Try again.`, retries successfully, and asserts exactly one expected browser resource error from the mocked 503 while all other console/page errors remain fatal.
- Accessibility and side-effect guards: every scenario keeps strict console/page guards, blocks paid-provider/Ollama calls, intercepts every app API request, and runs `@axe-core/playwright` with zero critical violations after the relevant interactive states are visible.
- Unit hardening: `frontend/__tests__/course-reader.test.tsx` now mocks the reader's `getReviewQueue` dependency and asserts the reader asks for all-card review metadata for the active chapter's section cards.
- Backend stability: `tests/test_review.py`, `tests/test_learner_scoping.py`, and `tests/test_review_availability_service.py` passed three consecutive runs: `44 passed, 8 warnings` each.
- Frontend stability: `tsc --noEmit` passed; `eslint` passed; Vitest passed `102 files / 839 tests`.
- Release gate: `rtk ./build.sh` passed with backend `884 passed, 31 warnings`, generated artifacts drift check, frontend typecheck, Vitest `839 passed`, Next production build, and final `BUILD OK`.
