...
export default function WorkspaceModeMenu() {
  ...
  const menuPanelRef = useDialogFocus<HTMLDivElement>(open, { trap: false });
  const dialogRef = useDialogFocus<HTMLElement>(confirmInstructor);
  ...
  useDismissOnOutsideOrEscape(open, closeMenu, menuRef);
  useDismissOnOutsideOrEscape(confirmInstructor, closeDisclosure, dialogRef);
